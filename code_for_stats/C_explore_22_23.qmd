...
c_2022_compat <- c_2022_mess %>%
  select(c(2:5,18,19,23,6,7,10,13,20:22,17))
```
...
c_2023_compat <- c_2023_mess %>%
  select(c(22,23,25,2,27,38,39,5,7,10,11,13,16,18,21)) %>%
  replace(is.na(.), 0)
```
...
ggplot(c_22_23, aes(x=factor(year), y=sl_carbon_9m2, colour = interaction(gap, park_id, sep=':'))) + 
  geom_boxplot() + theme(legend.title = element_blank())
