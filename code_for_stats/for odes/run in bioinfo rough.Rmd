...
Py_init <- seq(0, 500, length.out = n_i) 
Pm_init <- rep(0, n_i)
U_init <- rep(100, n_i)
...
# add initial condition run using
...
names(args_id) <- names(arg_less_last)
...
# function
return_year_stable <- function(odes) {
...
year_stable <- lapply(args_recover, return_year_stable)

# add to simulation data
f_yr <- data.frame(id = parse_number(names(year_stable)), g_or_f = str_remove_all(names(year_stable), "[:digit:]"), year = unlist(year_stable)) %>% 
  left_join(args_id) %>% 
  mutate(Moose = unlist(map(initial_conditions, pluck, 1)), By = unlist(map(initial_conditions, pluck, 2))) %>% 
  na.omit() %>% 
  select(!c(1,4,5))
...
avg_yr_f <- f_yr %>% 
  ...
  summarise(mean_yr = mean(as.numeric(f_yr$year))) %>% 
  ungroup() %>% 
  mutate(Moose = as.numeric(as.character(`as.factor(Moose)`))) %>% 
  mutate(By = as.numeric(as.character(`as.factor(By)`))) %>% 
  select(c(3:5))
...
## functions
```{r}
# to repeat ode over initial conditions and parameter values
...
param_recovery <- function(parameter){
  ...
  param_plot <- ggplot(recover_df, aes(x = parameter, y = recover, colour = recover)) +
  geom_violin() +
...
# to return year stable for each parameter
