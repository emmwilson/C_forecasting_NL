...
n_fs_ratio_plot <- ggplot(n_fs_ratio, aes(ratio, percent_recover)) +
  geom_point() +
  xlim(c(0,50)) +
  xlab(expression(atop(paste(underline(Young~palatable~biomass~(t))), "\nMoose biomass (t)"))) +
  ylab('Percent simulations recover')

saveRDS(n_fs_ratio, file = "~/C_forecasting_NL/code_for_stats/for odes/outputs/n_fs_ratio.RDS") 

jpeg("~/C_forecasting_NL/code_for_stats/for odes/outputs/n_fs_ratio_plot.jpg", height = 13, width = 15, units = "cm", res = 600)
n_fs_ratio_plot
dev.off()
```
...
n_fs_ratio_plot <- ggplot(avg_yr_50_f_ratio, aes(ratio, mean_yr)) +
  geom_point() +
  xlab(expression(atop(paste(underline(Young~palatable~biomass~(t))), "\nMoose biomass (t)"))) +
