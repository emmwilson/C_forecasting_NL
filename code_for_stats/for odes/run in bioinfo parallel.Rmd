...
# run function
n_p_recoveries <- mclapply(n_p_list, rep_over_n_param_samples, mc.cores = 2)
...
args$output <- mcmapply(ode_reps, parameters, initial_conditions, SIMPLIFY = F, mc.cores = 3) # add progress bar or periodic outputs and try to make parallel
...
args_less_steady_NAs <- mclapply(args_less, return_year_stable_delta, mc.cores = 3) # this function returns NAs if the simulation doesn;t reach steady state

args_less_steady <- args_less[!is.na(args_less_steady_NAs)]


# # get final stocks from remaining simulations
arg_less_last <- mclapply(args_less_steady,tail, 1, SIMPLIFY = T, mc.cores = 3)
...
year_stable_delta <- mclapply(args_recover, return_year_stable_delta, mc.cores = 3)
...
```{r}
ode_recovery <- mclapply(names(parms), param_recovery, mc.cores = 4)
```
...
param_recovery <- mclapply(names(parms), return_year_stable_param, mc.cores = 4)
```
