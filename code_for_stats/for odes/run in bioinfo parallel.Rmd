...
n_p_list <- as.list(rgamma(20, 4, 0.0175), 1000)

n_p_list_test <- as.list(rgamma(10, 4, 0.07))


M_initt <- seq(0, 6, length.out = 15)
Py_initt <- seq(0, 300, length.out = 15) 
Pm_initt <- rep(0, 15)
U_initt <- rep(100, 15)
init_seqt <- data.frame(Moose=rep(M_initt, each = 15), Pyoung = rep(Py_initt, 15), Pmature = rep(Pm_initt,15), Unpal = rep(U_initt,15))

y_0dtest <- split(as.matrix(init_seqt), row(init_seqt)) %>% 
  lapply(setNames, nm = c("Moose", "Pyoung", "Pmature", "Unpal"))
...
# run function
n_p_recoveries <- future_lapply(n_p_list_test, rep_over_n_param_samples,  future.seed = TRUE)
...
args_less_steady_NAs <- future_lapply(args_less, return_year_stable_delta) # this function returns NAs if the simulation doesn;t reach steady state
...
jpeg("~/C_forecasting_NL/code_for_stats/for odes/outputs/heatmap_moose_By.jpg", height = 10, width = 13, units = "cm", res = 600)
heatmap_moose_By
dev.off()
...
ode_recovery <- future_lapply(names(parms2), param_recovery)

plot_ode_recovery <- future_lapply(ode_recovery, plot_effect_param)
```
