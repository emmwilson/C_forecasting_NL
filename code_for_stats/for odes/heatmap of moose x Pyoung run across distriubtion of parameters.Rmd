...
lower_rP <- 0.1 # maybe streamline this to draw from vector for heatmap
upper_rP <- 0.3

rP_list <- as.list(signif(seq(from = lower_rP, to = upper_rP, by = (upper_rP - lower_rP)/10),4)) # should I standardize each step change in parameter value, or shoud it be across teh reasonable values from literature
param_list_rp <- list()

for(i in 1:length(rP_list)){
  param_list_rp[[i]] <- parms
  param_list_rp[[i]][['rP']] <- rP_list[[i]]
}

g_vs_f_param_rP3 <- lapply(param_list_rp, run_ode_get_fvsg3)
names(g_vs_f_param_rP3) <- lapply(rP_list, pluck, 1)

g_vs_f_param_rP23 <- mapply(cbind, g_vs_f_param_rP3, "rP"=rP_list, SIMPLIFY=F)

g_vs_f_param_rP23_new <- do.call(rbind, Map(data.frame, g_vs_f_param_rP23)) %>% 
  mutate(rP = as.factor(rP), g_or_f = as.factor(g_or_f)) %>% 
  group_by(rP, g_or_f, .drop=FALSE) %>% 
  count(.drop = FALSE) %>% 
  filter(g_or_f == " forest") %>% 
  mutate(percent = n/nrow(g_vs_f_param_rP23[[1]]))

ggplot(g_vs_f_param_rP23_new, aes(x = rP, y = percent)) +
  geom_point()
...
lPm_list <- as.list(signif(seq(from = lower_lPm, to = upper_lPm, by = (upper_lPm - lower_lPm)/10),4))
param_list_lPm <- list()
...
kPm_list <- as.list(signif(seq(from = lower_kPm, to = upper_kPm, by = (upper_kPm - lower_kPm )/10),4))
param_list_kPm <- list()
...
lower_e <- 0.01 
upper_e <- 0.03 
e_list <- as.list(signif(seq(from = lower_e, to = upper_e, by = (upper_e - lower_e )/10),4))
param_list_e <- list()
...
lower_a <- 0.996 
upper_a <- 1.005 
a_list <- as.list(signif(seq(from = lower_a, to = upper_a, by = (upper_a - lower_a)/10),4)) # should I standardize each step change in parameter value, or shoud it be across teh reasonable values from literature
param_list_a <- list()
...
lM_list <- as.list(signif(seq(from = lower_lM, to = upper_lM, by = (upper_lM - lower_lM)/10),4)) # should I standardize each step change in parameter value, or shoud it be across teh reasonable values from literature
param_list_lM <- list()
...
s_list <- as.list(signif(seq(from = lower_s, to = upper_s, by = (upper_s - lower_s)/10),4)) # should I standardize each step change in parameter value, or shoud it be across teh reasonable values from literature
param_list_s <- list()
...
alphaUPy_list <- as.list(signif(seq(from = lower_aUPy, to = upper_aUPy, by = (upper_aUPy - lower_aUPy)/10),4)) # should I standardize each step change in parameter value, or shoud it be across teh reasonable values from literature
...
alphaUPm_list <- as.list(signif(seq(from = lower_aUPm, to = upper_aUPm, by = (upper_aUPm - lower_aUPm)/10),4)) # should I standardize each step change in parameter value, or shoud it be across teh reasonable values from literature
...
kPy_list <- as.list(signif(seq(from = lower_kPy, to = upper_kPy, by = (upper_kPy - lower_kPy)/10),4)) # should I standardize each step change in parameter value, or shoud it be across teh reasonable values from literature
param_list_kPy <- list()
...
g_list <- as.list(signif(seq(from = lower_g, to = upper_g, by = (upper_g - lower_g)/10),4)) # should I standardize each step change in parameter value, or shoud it be across teh reasonable values from literature
param_list_g <- list()
...
rU_list <- as.list(signif(seq(from = lower_rU, to = upper_rU, by = (upper_rU - lower_rU)/10),4)) # should I standardize each step change in parameter value, or shoud it be across teh reasonable values from literature
param_list_rU <- list()
...
alphaPyU_list <- as.list(signif(seq(from = lower_aPyU, to = upper_aPyU, by = (upper_aPyU - lower_aPyU)/10),4)) # should I standardize each step change in parameter value, or shoud it be across teh reasonable values from literature
...
alphaPmU_list <- as.list(signif(seq(from = lower_aPmU, to = upper_aPmU, by = (upper_aPmU - lower_aPmU)/10),4)) # should I standardize each step change in parameter value, or shoud it be across teh reasonable values from literature
...
kU_list <- as.list(signif(seq(from = lower_kU, to = upper_kU, by = (upper_kU - lower_kU)/10),4)) # should I standardize each step change in parameter value, or shoud it be across teh reasonable values from literature
param_list_kU<- list()
...
lU_list <- as.list(signif(seq(from = lower_lU, to = upper_lU, by = (upper_lU - lower_lU)/10),4)) # oen thing to keep in mind is that the growth rate would probably change along with the mortality
param_list_lU<- list()
