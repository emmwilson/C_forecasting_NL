...
```{r}
GM_env_raster_masked_scaled_bm <- rast("data created/GM_env_raster_masked_scale_bm.tif") # stack of enviornmental variables' rasters
TN_env_raster_masked_scaled_bm <- rast("data created/TN_env_raster_masked_scale_bm.tif")
```
...
values(TN_predict_tot_gap_transform_bm) <- 1 / (1 + exp(-values(TN_predict_tot_gap_bm))) # value is c per m2 so can multiply by 90/1000 to get absloute estimate of carbon (not densityt) per cell (90m2)
