...
c_env_sites_scale_GM_gap$gap <- as.numeric(c_env_sites_scale_GM_gap$gap)
...
c_env_sites_scale_TN_gap$gap <- as.numeric(c_env_sites_scale_TN_gap$gap)
```
...
```{r}
#run the model
GM_lm_null_gap <- bi_glm(c_env_sites_scale_GM_gap, "gap", "1")
```
...
# run the model
GM_lm_full_gap <- bi_glm(c_env_sites_scale_GM_gap, "gap", full_var)
```
...
## univariate
```{r}
GM_lm_uni_gap <- lapply(full_var, bi_glm, df = c_env_sites_scale_GM_gap, c = "gap")
...
GM_lm_full_gap2 <- glm(formula = gap ~ FHT + EVIamp + EVImed + ELE + SLO + ASP + LGS + Moose_new, data = c_env_sites_scale_GM_gap, family = binomial(link = "logit")) 
...
GM_red_gap_models_r <- list()
GM_red_gap_models_r[[names(GM_lm_uni_gap)[1]]] <- GM_lm_uni_gap[[1]]
GM_red_gap_models_r[[names(GM_lm_uni_gap)[3]]] <- GM_lm_uni_gap[[3]]
GM_red_gap_models_r[["GM_lm_red_gap"]] <- GM_lm_red_gap
Modnames_GM_red_gap <- paste(names(GM_red_gap_models_r))

# get table
aic_GMred_gap <- aictab(cand.set = GM_red_gap_models_r, modnames = Modnames_GM_red_gap, sort = TRUE) 
...
GM_lm_gap_summary <- rbind(GM_lm_null_gap_summary, GM_lm_full_gap_summary, GM_uni_gap_summary_all, GM_lm_red_gap_summary) 
...
# get table
aic_GM_gap <- as.data.frame(aictab(cand.set = GM_models_gap_r, modnames = Modnames_GM_gap_r, sort = TRUE))

#add R2 to AIC table
R2_GM_gap <- as.data.frame(sapply(GM_models_gap_r, R2)) %>% setNames(c("R2"))
R2_GM_gap <- R2_GM_gap %>% 
   mutate("Modnames" = rownames(R2_GM_gap))
R2_GM_gap
aic_GM_gap

aic_R2_GM_gap <- full_join(aic_GM_gap, R2_GM_gap) # table of AIC values for null, full and univariate models
```
...
```{r}
c_env_sites_scale_TN_gap$Moose_new <-replace_na(c_env_sites_scale_TN_gap$Moose_new, "cH")
```

## null
```{r}
#run the model
TN_lm_null_gap <- bi_glm(c_env_sites_scale_TN_gap, "gap", "1")
```
...
# run the model
TN_lm_full_gap <- bi_glm(c_env_sites_scale_TN_gap, "gap", full_var)
...
glm(gap ~ FHT +EVIamp + EVImed + ELE + SLO + ASP + LGS + Moose_new, data = c_env_sites_scale_TN_gap,
...
library(arm)
fit <- bayesglm(gap ~ FHT +EVIamp + EVImed + ELE + SLO + ASP + LGS + Moose_new, data=c_env_sites_scale_TN_gap, family=binomial(link='logit'))
display(fit)

library(BMA) # to help drop variables?
x <- c_env_sites_scale_TN_gap[11:18]
...
## univariate
```{r}
TN_lm_uni_gap <- lapply(full_var, bi_glm, df = c_env_sites_scale_TN_gap, c = "gap")
...
## reduced
```{r}
TN_lm_full_gap2 <- glm(gap ~ FHT +EVIamp + ELE + SLO + ASP , data = c_env_sites_scale_TN_gap,
                       family = binomial("logit"))
...
```{r}
# check uninformative priors
AICc(TN_lm_red_gap)
AICc(TN_lm_full_gap2) # compare to full model
...
TN_lm_red_gap2 <- glm(gap ~ FHT +EVIamp +  ASP , data = c_env_sites_scale_TN_gap,
                       family = binomial("logit"))
TN_lm_red_gap3 <- glm(gap ~ FHT +EVIamp +  ELE , data = c_env_sites_scale_TN_gap,
                       family = binomial("logit")) # dropping ASP bring AICc <2 away
confint(glm(gap ~ FHT +EVIamp +  ELE , data = c_env_sites_scale_TN_gap,
                       family = binomial("logit"))) # and overlaps 0


TN_lm_red_gap4 <- glm(gap ~ FHT +EVIamp, data = c_env_sites_scale_TN_gap,
                       family = binomial("logit")) # also <2 then next model
...
Modnames_TN_red_gap <- paste(names(TN_red_gap_models_r))

# get table
aic_TNred_gap <- aictab(cand.set = TN_red_gap_models_r, modnames = Modnames_TN_red_gap, sort = TRUE) 

# believe ELE and ASP uninformative

```

```{r}
TN_lm_red_gap_summary <- as.data.frame(summary(TN_lm_red_gap4)$coefficients) %>% 
...
```{r}
# combine summary dataframes for null, full and univariate models
TN_lm_gap_summary <- rbind(TN_lm_null_gap_summary, TN_lm_full_gap_summary, TN_uni_gap_summary_all, TN_lm_red_gap_summary) 
...
# get table
aic_TN_gap <- as.data.frame(aictab(cand.set = TN_models_gap_r, modnames = Modnames_TN_gap_r, sort = TRUE))

#add R2 to AIC table
R2_TN_gap <- as.data.frame(sapply(TN_models_gap_r, R2)) %>% setNames(c("R2"))
R2_TN_gap <- R2_TN_gap %>% 
   mutate("Modnames" = rownames(R2_TN_gap))
R2_TN_gap
aic_TN_gap

aic_R2_TN_gap <- full_join(aic_TN_gap, R2_TN_gap) # table of AIC values for null, full and univariate models
```
