...
```{r}
GM_env_raster_masked_scale_bm <- rast("data created/GM_env_raster_masked_scale_bm.tif")
TN_env_raster_masked_scale_bm <- rast("data created/TN_env_raster_masked_scale_bm.tif")
```
...
values(GM_predict_tot_transform_bm) <- exp(values(GM_predict_tot_bm))*90/1000 # value is c per m2 so can multiply by 90/1000 to get absloute estimate of carbon (not densityt) per cell (90m2)
#plot(GM_predict_tot_transform, background = "lightblue") 
...
values(TN_predict_tot_transform_bm) <- exp(values(TN_predict_tot_bm))*90/1000 # value is c per m2 so can multiply by 90/1000 to get absloute estimate of carbon (not densityt) per cell (90m2)
#plot(TN_predict_tot_transform, background = "lightblue") 
