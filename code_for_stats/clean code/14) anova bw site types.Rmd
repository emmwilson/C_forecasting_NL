...
c_22_23_mi2 <- rbind(subset(dplyr::select(c_22_23_moosegap, c(1,5,6,16))), dplyr::select(c_22_23_insect2, c(1,5,6,15)))
...
```{r}
# both parks together
disturbance_all_model <- aov(new_tot~gap, data = c_22_23_mi2)

anova(disturbance_all_model)
TukeyHSD(disturbance_all_model)

# GM
disturbance_GM_model <- aov(new_tot~gap, data = subset(c_22_23_mi2, park_id == "GM"))

anova(disturbance_GM_model)
TukeyHSD(disturbance_GM_model)

# TN
disturbance_TN_model <- aov(new_tot~gap, data = subset(c_22_23_mi2, park_id == "TN"))
