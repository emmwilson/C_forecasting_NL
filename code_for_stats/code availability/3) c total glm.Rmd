...
```{r}
c_22_23 <- read_xlsx("data required/c_22_23.xlsx") %>% 
  filter(ex_ctrl != "EX")
...
c_m2_sites <- c_22_23 %>%
  ...
  summarize(c_m2_site = sum(total_c_per_subplot)/36) # divide by 36 because each of the 4 subplots is 9m2 to get m2

write.xlsx(c_m2_sites, file = "data created/c_m2_site.xlsx")
...
c_env_sites <- full_join(c_m2_sites, env_site_final) %>%
  dplyr::select(!c(FAC, SPC, CC, CEC_LCT)) # remove unwanted columns based on correlation analysis
```
