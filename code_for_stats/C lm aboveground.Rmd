...
thoughts: FHT, EVIamp, EVImed and LGS all make the top models. 

There was the addition of aspect in the top models before I updated ASP (used to be between 0 and 360), the AIC got much higher if aspect gets removed (model 10 was the best model without aspect and its deltaAIC was 20). I think I needed to change the way aspect was included, when I first included aspect 360 was the furthest away from 0 (north), when in reality that is the closest you can get to north. I would expect the relationship between C and aspect to be unimodal but the model is constrained to linear. If we change it to absolute number of degrees from 0, N would always be 0, S would be 180, E  and W would be 90. Do you think that works?

*models above run with abs(180 - ASP) which is basically just saying the number of degrees from south. this makes ASP less important
...
know this is true for TN so trying for GM
