...
```{r}
env_sites_GM <- read_xlsx("/Volumes/Emmerson's backup/C_forecasting_NL_env_data/All in NAD 83 - UTM 21/Sites_and_subplots/env_var/xlsx/env_sites_GM.xlsx") %>% # environmental data from Gros Morne sites
  rename(CC = "CC10fill")

env_sites_TN <- read_xlsx("/Volumes/Emmerson's backup/C_forecasting_NL_env_data/All in NAD 83 - UTM 21/Sites_and_subplots/env_var/xlsx/env_sites_TN.xlsx") %>% #environmental data from Terra Nova sites
    rename(CC = "CC10fill")
...
```{r}
env_subplots_GM <- read_xlsx("/Volumes/Emmerson's backup/C_forecasting_NL_env_data/All in NAD 83 - UTM 21/Sites_and_subplots/env_var/xlsx/env_subplots_GM.xlsx") %>% # environmental data from Gros Morne subplots
  rename(CC = "CC15fill")

env_subplots_TN <- read_xlsx("/Volumes/Emmerson's backup/C_forecasting_NL_env_data/All in NAD 83 - UTM 21/Sites_and_subplots/env_var/xlsx/env_subplots_TN.xlsx") %>% #environmental data from Terra Nova subplots
    rename(CC = "CC10fill")
...
# seperate by park
dominant_sp_TN <- select(dominant_sp, c(10, 9))
dominant_sp_GM <- select(dominant_sp, c(10, 6))
dominant_sp_out <- select(dominant_sp, c(10, 3))
...
```{r}
# Terra Nova
# rename SPC column to name in raster
dominant_sp_TN_raster <- dominant_sp_TN %>% 
  rename(SPC_TN = "SPC")

# reclassify values in first column of dominant_sp... to values in second column
SPD_TN <- classify(SPC_TN, dominant_sp_TN_raster)

#same for Gross Morne
dominant_sp_GM_raster <- dominant_sp_GM %>% 
  rename(SPC_GM = "SPC")

SPD_GM <- classify(SPC_GM, dominant_sp_GM_raster)


TN_env_list <- list(SPD_TN, FAC_TN, EVIamp_TN, EVImed_TN, FHT_TN, CC10fill_TN, SLO_TN, ELE_TN, ASP_TN, LCT_TN)

TN_env_raster <- rast(TN_env_list)

GM_env_list <- list(SPD_GM, FAC_GM, EVIamp_GM, EVImed_GM, FHT_GM, CC15fill_GM, SLO_GM, ELE_GM, ASP_GM, LCT_GM)
...
```{r, warning=FALSE}
predictors_site_env <- env_sites_SPD[, -c(1:7, 18)]
...
predictors_site_env_GM <- GM_env_sites[, -c(1:7, 17, 18)]
# correlation structure
...
predictors_site_env_TN <- TN_env_sites[, -c(1:7, 18)]
# correlation structure
