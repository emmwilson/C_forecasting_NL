...
names(GM_lm_red) <- GM_red_list

GM_lm_red_summary <- lapply(GM_lm_red, summary)
GM_lm_red_summary

Modnames_GM_red_r <- names(GM_lm_red)

aictab(cand.set = GM_lm_red, modnames = Modnames_GM_red_r, sort = TRUE) #this calculates corrected AIC so slightly different values than normal AIC

aictab(cand.set = GM_lm_red, modnames = Modnames_GM_red_r, sort = TRUE)
```
...
view(aictab(cand.set = GM_models_site_sub_r, modnames = Modnames_GM_site_sub_r, sort = TRUE))
```
...
names(GM_sub_lm_red) <- GM_sub_red_list
...
aictab(cand.set = GM_sub_lm_red, modnames = Modnames_GM_sub_red_r, sort = TRUE) #this calculates corrected AIC so slightly different values than normal AIC

view(aictab(cand.set = GM_sub_lm_red, modnames = Modnames_GM_sub_red_r, sort = TRUE))

GM_sub_lm_red_r2 <- lapply(GM_sub_lm_red, r.squaredGLMM) # none have the same r2m and r2c values
```
...
# get table
view(aictab(cand.set = TN_models_r, modnames = Modnames_TN_r, sort = TRUE))
...
aictab(cand.set = TN_lm_red, modnames = Modnames_TN_red_r, sort = TRUE) #this calculates corrected AIC so slightly different values than normal AIC

aictab(cand.set = TN_lm_red, modnames = Modnames_TN_red_r, sort = TRUE)
...
r.squaredGLMM(TN_lm_null_site_sub)
```
...
names(TN_sub_lm_red) <- TN_sub_red_list
...
aictab(cand.set = TN_sub_lm_red, modnames = Modnames_TN_sub_red_r, sort = TRUE) #this calculates corrected AIC so slightly different values than normal AIC

view(aictab(cand.set = TN_sub_lm_red, modnames = Modnames_TN_sub_red_r, sort = TRUE))

TN_sub_lm_red_r2 <- lapply(TN_sub_lm_red, r.squaredGLMM) # none have the same r2m and r2c values
```
