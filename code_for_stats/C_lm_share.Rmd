...
view(aic_GM) # table of AIC values for null, full and univariate models
...
GM_red_list <- list("FHT", GM_red_list1, GM_red_list2, GM_red_list3, GM_red_list4, GM_red_list5, GM_red_list6, GM_red_list7, GM_red_list8, GM_red_list9)
...
# R2_GM_red <- R2_GM_red %>% 
#   mutate("Modnames" = rownames(R2_GM_red))
# 
# aic_R2_GM_red <- full_join(aic_GM_red, R2_GM_red)
# 
# 
# # add reduced, univariate, full, and null models then get aic values
# aic_GM_all <- as.data.frame(aictab(cand.set = c(GM_models_r, GM_lm_red), modnames = c(Modnames_GM_r, Modnames_GM_red_r), sort = TRUE))
# 
# aic_R2_GM_all <- left_join(aic_GM_all, rbind(R2_GM_red, R2_GM), by = "Modnames")
# 
# view(aic_R2_GM_all) #compares AIC of all models tested
```
...
GM_lm_null_site_sub_summary <- summary(GM_lm_null_site_sub)
r.squaredGLMM(GM_lm_null_site_sub)
```
...
r.squaredGLMM(GM_lm_full_site_sub)
```
...
GM_lm_uni_site_sub_r2 <- lapply(GM_lm_uni_site_sub, r.squaredGLMM)
GM_lm_uni_site_sub_r2s
```
...
# get table
aic_GM_sub_fe <- aictab(cand.set = GM_models_sub_r, modnames = Modnames_GM_sub_r, sort = TRUE) %>% 
  mutate(R2m = sapply(lapply(GM_models_sub_r, r.squaredGLMM), "[[", 1)) %>% 
  mutate(R2c = sapply(lapply(GM_models_sub_r, r.squaredGLMM), "[[", 2))
...
# get table
 aic_GM_sub <- aictab(cand.set = GM_models_site_sub_r, modnames = Modnames_GM_site_sub_r, sort = TRUE) 
#   mutate(R2m = sapply(lapply(GM_models_site_sub_r, r.squaredGLMM), "[[", 1)) %>% 
#   mutate(R2c = sapply(lapply(GM_models_site_sub_r, r.squaredGLMM), "[[", 2))
view(aic_GM_sub)
```
...
GM_sub_red_list <- list(full_var, "FHT", GM_sub_red_list1, GM_sub_red_list2, GM_sub_red_list3, GM_sub_red_list4, GM_sub_red_list5, GM_sub_red_list6, GM_sub_red_list7, GM_sub_red_list8, GM_sub_red_list9, GM_sub_red_list10)
...
aic_GM_sub_red <- aictab(cand.set = GM_sub_lm_red, modnames = Modnames_GM_sub_red_r, sort = TRUE) 
  # mutate(R2m = sapply(lapply(GM_sub_lm_red, r.squaredGLMM), "[[", 1)) %>% 
  # mutate(R2c = sapply(lapply(GM_sub_lm_red, r.squaredGLMM), "[[", 2))

# table for all models
aic_GM_sub_all <- aictab(cand.set = c(GM_models_site_sub_r, GM_sub_lm_red), modnames = c(Modnames_GM_site_sub_r, Modnames_GM_sub_red_r), sort = TRUE) 
  # mutate(R2m = sapply(lapply(c(GM_models_site_sub_r, GM_sub_lm_red), r.squaredGLMM), "[[", 1)) %>% 
  # mutate(R2c = sapply(lapply(c(GM_models_site_sub_r, GM_sub_lm_red), r.squaredGLMM), "[[", 2))
...
TN_red_list <- list("FHT", TN_red_list1, TN_red_list2, TN_red_list3, TN_red_list4, TN_red_list6, TN_red_list7, TN_red_list8, TN_red_list9, TN_red_list10, TN_red_list11)
...
aic_TN_red <- aictab(cand.set = TN_lm_red, modnames = Modnames_TN_red_r, sort = TRUE) %>% #this calculates corrected AIC so slightly different values than normal AIC
  mutate(R2 = lapply(TN_lm_red, R2))
...
r.squaredGLMM(TN_lm_full_site_sub)
```
...
TN_lm_uni_site_sub_r2 <- lapply(TN_lm_uni_site_sub, r.squaredGLMM)
TN_lm_uni_site_sub_r2
```
...
# get table
aic_TN_sub_fe <- aictab(cand.set = TN_models_sub_r, modnames = Modnames_TN_sub_r, sort = TRUE)%>% 
  mutate(R2m = sapply(lapply(TN_models_sub_r, r.squaredGLMM), "[[", 1)) %>% 
  mutate(R2c = sapply(lapply(TN_models_sub_r, r.squaredGLMM), "[[", 2))
...
# get table
aic_TN_sub <- aictab(cand.set = TN_models_site_sub_r, modnames = Modnames_TN_site_sub_r, sort = TRUE) 
  # mutate(R2m = sapply(lapply(TN_models_site_sub_r, r.squaredGLMM), "[[", 1)) %>% 
  # mutate(R2c = sapply(lapply(TN_models_site_sub_r, r.squaredGLMM), "[[", 2))
view(aic_TN_sub)
```
...
TN_sub_red_list <- list(full_var, "FHT", TN_sub_red_list1)
...
aic_TN_sub_red <- aictab(cand.set = TN_sub_lm_red, modnames = Modnames_TN_sub_red_r, sort = TRUE) 
  # mutate(R2m = sapply(lapply(TN_sub_lm_red, r.squaredGLMM), "[[", 1)) %>% 
  # mutate(R2c = sapply(lapply(TN_sub_lm_red, r.squaredGLMM), "[[", 2))

aic_TN_sub_all <- aictab(cand.set = c(TN_models_site_sub_r, TN_sub_lm_red), modnames = c(Modnames_TN_site_sub_r, Modnames_TN_sub_red_r), sort = TRUE)  
  # mutate(R2m = sapply(lapply(c(TN_models_site_sub_r, TN_sub_lm_red), r.squaredGLMM), "[[", 1)) %>% 
  # mutate(R2c = sapply(lapply(c(TN_models_site_sub_r, TN_sub_lm_red), r.squaredGLMM), "[[", 2))
